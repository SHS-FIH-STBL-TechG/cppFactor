...
# -*- coding: utf-8 -*-
"""
封装 m_vpc_mut_ty_log1 因子计算：
- 从 config.ini 读取路径与窗口参数
- 仅负责滑动拼接固定窗口数据
- 调用 m_vpc_mut_ty_log1 原算子输出结果
"""
...
from m_vpc_mut_ty_log1_factor import m_vpc_mut_ty_log1
...
def read_config(config_path: str) -> dict:
    ...
    section = "6001_m_vpc_mut_ty_log1"
...
def main():
    try:
        ...
        print(f"[{timestamp}] === 6001_m_vpc_mut_ty_log1 因子计算（Python封装） ===")
        ...
        # 因子实例仅负责计算，不再做配置解析
        factor = m_vpc_mut_ty_log1(lag=lag_window)
        ...
        with open(output_path, "w", encoding="utf-8") as output_file:
            ...
            for stock_idx in range(num_stocks):
                header += f",stock_{stock_idx}_mut_ty_log1"
