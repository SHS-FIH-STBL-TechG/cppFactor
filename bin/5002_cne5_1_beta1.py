...
# -*- coding: utf-8 -*-
"""
封装barra_cne5_1_beta1因子计算，从config.ini读取配置，调用原算子计算，输出C++格式结果
"""
...
# 导入本地算子（简化版，无平台依赖）
from barra_cne5_1_beta1_factor import barra_cne5_1_beta1
...
def main():
    ...
    try:
        ...
        # 读取配置
        print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] === 5002_cne5_1_beta1 因子计算（Python封装） ===")
        ...
        # 创建因子实例，传入配置参数
        factor = barra_cne5_1_beta1(lag=lag_window, reform_window=reform_window)
