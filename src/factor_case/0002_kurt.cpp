...
class Factor_0002_Kurt : public BaseFactor {
public:
    VectorXd calc_single(Tool::Database& database) override {
        ...
        bool unbiased = config.getBool("0002_kurt", "unbiased", true);  // 默认无偏
        
        cout << "=== 0002_Kurt 因子计算 ===" << endl;
        cout << "输入数据维度: " << data.rows() << " x " << data.cols() << endl;
        cout << "算法类型: " << (unbiased ? "无偏" : "有偏") << endl;
        
        // 计算峰度
        VectorXd result;
        BaseFactor::col_nankurt(data, result, unbiased);
...
int main() {
    try {
        // 读取配置文件
        Tool::ConfigReader config("config.ini");
        std::string input_csv = config.getString("0002_kurt", "input_csv", "");
        std::string output_csv = config.getString("0002_kurt", "output_csv", "");
        int precision = config.getInt("0002_kurt", "precision", 6);
        ...
        if (output_file.is_open()) {
            ...
            for (int i = 0; i < result.size(); ++i) {
                if (std::isnan(result[i])) {
                    output_file << "col_" << i << "_kurt,nan\n";
                } else {
                    output_file << "col_" << i << "_kurt," << std::fixed << std::setprecision(precision) << result[i] << "\n";
                }
            }
            output_file.close();
