...
class Factor_0006_Std : public BaseFactor {
public:
    VectorXd calc_single(Tool::Database& database) override {
        ...
        bool unbiased = config.getBool("0006_std", "unbiased", true);  // 默认无偏
        
        cout << "=== 0006_Std 因子计算 ===" << endl;
        cout << "输入数据维度: " << data.rows() << " x " << data.cols() << endl;
        cout << "算法类型: " << (unbiased ? "无偏" : "有偏") << endl;
        
        // 计算标准差
        VectorXd result;
        BaseFactor::col_nanstd(data, result, unbiased);
...
int main() {
    try {
        // 读取配置文件
        Tool::ConfigReader config("config.ini");
        std::string input_csv = config.getString("0006_std", "input_csv", "");
        std::string output_csv = config.getString("0006_std", "output_csv", "");
        int precision = config.getInt("0006_std", "precision", 6);
        ...
        if (output_file.is_open()) {
            ...
            for (int i = 0; i < result.size(); ++i) {
                if (std::isnan(result[i])) {
                    output_file << "col_" << i << "_std,nan\n";
                } else {
                    output_file << "col_" << i << "_std," << std::fixed << std::setprecision(precision) << result[i] << "\n";
                }
            }
            output_file.close();
