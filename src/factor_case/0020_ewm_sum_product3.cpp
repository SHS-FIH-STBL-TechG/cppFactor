...
int main() {
    try {
        ...
        const std::string initial_x_csv = config.getString("0020_ewm_sum_product3", "initial_x_csv", "");
        const std::string initial_y_csv = config.getString("0020_ewm_sum_product3", "initial_y_csv", "");
        const std::string initial_z_csv = config.getString("0020_ewm_sum_product3", "initial_z_csv", "");
        const std::string update_x_csv = config.getString("0020_ewm_sum_product3", "update_x_csv", "");
        const std::string update_y_csv = config.getString("0020_ewm_sum_product3", "update_y_csv", "");
        const std::string update_z_csv = config.getString("0020_ewm_sum_product3", "update_z_csv", "");
        const std::string weight_csv = config.getString("0020_ewm_sum_product3", "weight_csv", "");
        const std::string output_csv = config.getString("0020_ewm_sum_product3", "output_csv", "");
        const int precision = config.getInt("0020_ewm_sum_product3", "precision", 6);

        if (initial_x_csv.empty() || initial_y_csv.empty() || initial_z_csv.empty() ||
            update_x_csv.empty() || update_y_csv.empty() || update_z_csv.empty() ||
            weight_csv.empty() || output_csv.empty()) {
        ...
        database.loadFromCSV("initial_z", initial_z_csv);
        database.loadFromCSV("update_x", update_x_csv);
        database.loadFromCSV("update_y", update_y_csv);
        database.loadFromCSV("update_z", update_z_csv);
        ...
        MatrixXd initial_z = database.getMatrix("initial_z");
        MatrixXd update_x = database.getMatrix("update_x");
        MatrixXd update_y = database.getMatrix("update_y");
        MatrixXd update_z = database.getMatrix("update_z");
        MatrixXd weight_matrix = database.getMatrix("weight");

        if (initial_x.cols() == 0 || initial_y.cols() == 0 || initial_z.cols() == 0 ||
            weight_matrix.cols() == 0) {
        ...
        Ve initial_values_z = columnToVector(initial_z, 0);
        ...
        auto data_cache_z = OnlineBaseFactor::createOnlineBaseF<OnlineDataCache>(initial_values_z);
        auto ewm_sum_product3 = OnlineBaseFactor::createOnlineBaseF<OnlineEWMSumProduct3>(
            OnlineEWMSumProduct3::Window{weight_cache, data_cache_x, data_cache_y, data_cache_z});
        ...
        cout << "[" << Tool::Timestamp::getCurrentTimestamp() << "] === 0020_EWM_SUM_PRODUCT3 测试 ===" << endl;
        cout << "初始窗口长度: " << initial_values_x.size() << endl;

        const double init_value = ewm_sum_product3->getValue();
        ...
        const int update_rows = std::min({update_x.rows(), update_y.rows(), update_z.rows()});
        for (int row = 0; row < update_rows; ++row) {
            Ve update_values_x = rowToVector(update_x, row);
            Ve update_values_y = rowToVector(update_y, row);
            Ve update_values_z = rowToVector(update_z, row);
            if (update_values_x.size() == 0 || update_values_y.size() == 0 || update_values_z.size() == 0) {
                continue;
            }
            if (update_values_x.size() != update_values_y.size() || update_values_x.size() != update_values_z.size()) {
                std::cerr << "错误: 更新数据长度不一致" << std::endl;
                return 1;
            }

            const size_t version = static_cast<size_t>(row + 1);
            ewm_sum_product3->update(update_values_x, update_values_y, update_values_z, version);
            const double current_value = ewm_sum_product3->getValue();
