...
public:
    VectorXd calc_single(Tool::Database& database) {
        ...
        cout << "=== 0011_SRCM 因子计算 ===" << endl;
        cout << "输入数据维度: " << data.rows() << " x " << data.cols() << endl;
        
        // 计算半段排序自相关系数
        VectorXd result;
        BusinessFactor::col_split_rankcorr(data, result);
        
        cout << "各列半段排序自相关系数结果:" << endl;
...
int main() {
    try {
        ...
        std::string input_csv = config.getString("0011_srcm", "input_csv", "");
        std::string output_csv = config.getString("0011_srcm", "output_csv", "");
        int precision = config.getInt("0011_srcm", "precision", 6);
        ...
        if (output_file.is_open()) {
            ...
            for (int i = 0; i < result.size(); ++i) {
                if (std::isnan(result[i])) {
                    output_file << "col_" << i << "_srcm,nan\n";
                } else {
                    output_file << "col_" << i << "_srcm," << std::fixed << std::setprecision(precision) << result[i] << "\n";
                }
            }
            output_file.close();
