...
int main() {
    try {
        ...
        const std::string initial_x_csv = config.getString("0019_ewm_cov", "initial_x_csv", "");
        const std::string initial_y_csv = config.getString("0019_ewm_cov", "initial_y_csv", "");
        const std::string update_x_csv = config.getString("0019_ewm_cov", "update_x_csv", "");
        const std::string update_y_csv = config.getString("0019_ewm_cov", "update_y_csv", "");
        const std::string weight_csv = config.getString("0019_ewm_cov", "weight_csv", "");
        const std::string output_csv = config.getString("0019_ewm_cov", "output_csv", "");
        const int precision = config.getInt("0019_ewm_cov", "precision", 6);
        ...
        auto data_cache_mean_x = OnlineBaseFactor::createOnlineBaseF<OnlineDataCache>(initial_values_x);
        auto ewm_sum_x = OnlineBaseFactor::createOnlineBaseF<OnlineEWMSum>(OnlineEWMSum::Window{weight_cache, data_cache_mean_x});
        auto ewm_mean_x = OnlineBaseFactor::createOnlineBaseF<OnlineEWMMean>(OnlineEWMMean::Window{weight_cache, ewm_sum_x});

        auto data_cache_mean_y = OnlineBaseFactor::createOnlineBaseF<OnlineDataCache>(initial_values_y);
        auto ewm_sum_y = OnlineBaseFactor::createOnlineBaseF<OnlineEWMSum>(OnlineEWMSum::Window{weight_cache, data_cache_mean_y});
        auto ewm_mean_y = OnlineBaseFactor::createOnlineBaseF<OnlineEWMMean>(OnlineEWMMean::Window{weight_cache, ewm_sum_y});

        auto data_cache_prod_x = OnlineBaseFactor::createOnlineBaseF<OnlineDataCache>(initial_values_x);
        auto data_cache_prod_y = OnlineBaseFactor::createOnlineBaseF<OnlineDataCache>(initial_values_y);
        auto ewm_sum_product_xy = OnlineBaseFactor::createOnlineBaseF<OnlineEWMSumProduct>(
            OnlineEWMSumProduct::Window{weight_cache, data_cache_prod_x, data_cache_prod_y});

        auto ewm_cov = OnlineBaseFactor::createOnlineBaseF<OnlineEWMCov>(
            OnlineEWMCov::Window{weight_cache, ewm_sum_product_xy, ewm_mean_x, ewm_mean_y});
        ...
        cout << "[" << Tool::Timestamp::getCurrentTimestamp() << "] === 0019_EWM_COV 测试 ===" << endl;
        cout << "初始窗口长度: " << initial_values_x.size() << endl;

        const double init_value = ewm_cov->getValue();
        output_file << 0 << ",init," << std::fixed << std::setprecision(precision) << init_value << "\n";
        cout << "初始带权协方差: " << std::fixed << std::setprecision(precision) << init_value << endl;
        ...
        for (int row = 0; row < update_rows; ++row) {
            ...
            ewm_cov->update(update_values_x, update_values_y, version);
            const double current_value = ewm_cov->getValue();
            output_file << (row + 1) << ",update," << std::fixed << std::setprecision(precision) << current_value << "\n";
            cout << "更新 " << (row + 1) << " -> 带权协方差: " << std::fixed << std::setprecision(precision) << current_value << endl;
        }
