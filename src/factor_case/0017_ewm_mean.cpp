...
int main() {
    try {
        ...
        const std::string initial_csv = config.getString("0017_ewm_mean", "initial_csv", "");
        const std::string update_csv = config.getString("0017_ewm_mean", "update_csv", "");
        const std::string weight_csv = config.getString("0017_ewm_mean", "weight_csv", "");
        const std::string output_csv = config.getString("0017_ewm_mean", "output_csv", "");
        const int precision = config.getInt("0017_ewm_mean", "precision", 6);
        ...
        cout << "[" << Tool::Timestamp::getCurrentTimestamp() << "] === 0017_EWM_MEAN 测试 ===" << endl;
        cout << "初始窗口长度: " << initial_values.size() << endl;

        const double init_value = ewm_mean->getValue();
        output_file << 0 << ",init," << std::fixed << std::setprecision(precision) << init_value << "\n";
        cout << "初始带权均值: " << std::fixed << std::setprecision(precision) << init_value << endl;
        ...
        for (int row = 0; row < update_matrix.rows(); ++row) {
            ...
            ewm_mean->update(step_values, version);
            const double current_value = ewm_mean->getValue();
            output_file << (row + 1) << ",update," << std::fixed << std::setprecision(precision) << current_value << "\n";
            cout << "更新 " << (row + 1) << " -> 带权均值: " << std::fixed << std::setprecision(precision) << current_value << endl;
        }
