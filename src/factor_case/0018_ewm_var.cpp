...
int main() {
    try {
        ...
        const std::string initial_csv = config.getString("0018_ewm_var", "initial_csv", "");
        const std::string update_csv = config.getString("0018_ewm_var", "update_csv", "");
        const std::string weight_csv = config.getString("0018_ewm_var", "weight_csv", "");
        const std::string output_csv = config.getString("0018_ewm_var", "output_csv", "");
        const int precision = config.getInt("0018_ewm_var", "precision", 6);
        ...
        auto ewm_sum = OnlineBaseFactor::createOnlineBaseF<OnlineEWMSum>(OnlineEWMSum::Window{weight_cache, data_cache_sum});
        auto ewm_mean = OnlineBaseFactor::createOnlineBaseF<OnlineEWMMean>(OnlineEWMMean::Window{weight_cache, ewm_sum});
        ...
        auto ewm_var = OnlineBaseFactor::createOnlineBaseF<OnlineEWMVar>(
            OnlineEWMVar::Window{weight_cache, ewm_mean, ewm_sum_product});
        ...
        cout << "[" << Tool::Timestamp::getCurrentTimestamp() << "] === 0018_EWM_VAR 测试 ===" << endl;
        cout << "初始窗口长度: " << initial_values.size() << endl;

        const double init_value = ewm_var->getValue();
        output_file << 0 << ",init," << std::fixed << std::setprecision(precision) << init_value << "\n";
        cout << "初始带权方差: " << std::fixed << std::setprecision(precision) << init_value << endl;
        ...
        for (int row = 0; row < update_matrix.rows(); ++row) {
            ...
            ewm_var->update(step_values, version);
            const double current_value = ewm_var->getValue();
            output_file << (row + 1) << ",update," << std::fixed << std::setprecision(precision) << current_value << "\n";
            cout << "更新 " << (row + 1) << " -> 带权方差: " << std::fixed << std::setprecision(precision) << current_value << endl;
        }
