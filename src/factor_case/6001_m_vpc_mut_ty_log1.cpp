...
int main() {
    try {
        ...
        std::string input_amt = config.getString("6001_m_vpc_mut_ty_log1", "input_amt", "");
        std::string input_close = config.getString("6001_m_vpc_mut_ty_log1", "input_close", "");
        std::string output_csv = config.getString("6001_m_vpc_mut_ty_log1", "output_csv", "");

        int precision = config.getInt("6001_m_vpc_mut_ty_log1", "precision", 6);
        int stepSize = config.getInt("6001_m_vpc_mut_ty_log1", "stepSize", 1);
        int lagWindow = config.getInt("6001_m_vpc_mut_ty_log1", "lagWindow", 120);
        ...
        cout << "[" << Tool::Timestamp::getCurrentTimestamp() << "] === 6001_m_vpc_mut_ty_log1 因子计算 ===" << endl;
        ...
        for (int i = 0; i < num_stocks; ++i) {
            output_file << ",stock_" << i << "_mut_ty_log1";
        }
        output_file << "\n";
        ...
        {
            PROFILE_SCOPE("m_vpc_mut_ty_log1::Init");
        ...
        while (current_time < num_timepoints) {
            ...
            {
                PROFILE_SCOPE("m_vpc_mut_ty_log1::Update");
