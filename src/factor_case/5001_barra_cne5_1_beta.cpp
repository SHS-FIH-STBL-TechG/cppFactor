...
int main() {
    try {
        ...
        // 读取输入文件路径
        std::string input_pct_chg = config.getString("5001_cne5_1_beta", "input_pct_chg", "");
        std::string input_a_mkt_cap = config.getString("5001_cne5_1_beta", "input_a_mkt_cap", "");
        std::string input_is_valid = config.getString("5001_cne5_1_beta", "input_is_valid", "");
        std::string output_csv = config.getString("5001_cne5_1_beta", "output_csv", "");
        
        // 读取配置参数
        int precision = config.getInt("5001_cne5_1_beta", "precision", 6);
        int stepSize = config.getInt("5001_cne5_1_beta", "stepSize", 1);
        int lagWindow = config.getInt("5001_cne5_1_beta", "lagWindow", 100);
        int reformWindow = config.getInt("5001_cne5_1_beta", "reformWindow", 1);
        ...
        cout << "[" << Tool::Timestamp::getCurrentTimestamp() << "] === 5001_cne5_1_beta 因子计算 ===" << endl;
        ...
        {
            PROFILE_SCOPE("barra_cne5_1_beta::Init");
        ...
        while (current_time < num_timepoints) {
            ...
            // 调用Update
            {
                PROFILE_SCOPE("barra_cne5_1_beta::Update");
