...
int main() {
    try {
        ...
        std::string input_csv = config.getString("0010_dm", "input_csv", "");
        std::string output_csv = config.getString("0010_dm", "output_csv", "");
        int precision = config.getInt("0010_dm", "precision", 6);
        ...
        if (output_file.is_open()) {
            ...
            for (int i = 0; i < result.size(); ++i) {
                if (std::isnan(result[i])) {
                    output_file << "col_" << i << "_dm,nan\n";
                } else {
                    output_file << "col_" << i << "_dm," << std::fixed << std::setprecision(precision) << result[i] << "\n";
                }
            }
            output_file.close();
