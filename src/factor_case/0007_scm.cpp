...
int main() {
    try {
        ...
        std::string input_csv = config.getString("0007_scm", "input_csv", "");
        std::string output_csv = config.getString("0007_scm", "output_csv", "");
        int precision = config.getInt("0007_scm", "precision", 6);
        ...
        if (output_file.is_open()) {
            ...
            for (int i = 0; i < result.size(); ++i) {
                if (std::isnan(result[i])) {
                    output_file << "col_" << i << "_scm,nan\n";
                } else {
                    output_file << "col_" << i << "_scm," << std::fixed << std::setprecision(precision) << result[i] << "\n";
                }
            }
            output_file.close();
