...
class m_vpc_mut_ty_log19(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        rtns_rpl = mclose.values / mopen.values - 1
        rtns_rpl = rtns_rpl[-237 * 5:, :]
        mclose_rpl = mclose.values[-237 * 5:, :]  # 取出n天

        alpha = list(map(lambda i: self.cal_num_greenback(mclose_rpl[:, i], rtns_rpl[:, i]), range(rtns_rpl.shape[1])))
    ...
    @staticmethod
    def cal_num_greenback(closee, rtns):
        if np.nanmin(closee) == closee[0]:
            num_greenback = np.nan
        else:
            rtns_low = np.nanmin(closee) / closee[0] - 1
            idx_low = np.argmin(closee)  # 最低点的位置
            rtns_sort = np.sort(rtns[:idx_low])  # 从小到大排序
            rtns_cumsum = np.nancumsum(rtns_sort)
            num_greenback = np.where(rtns_cumsum < rtns_low)[0]
            if len(num_greenback) > 0:
                num_greenback = num_greenback[0] / len(closee)
            else:
                num_greenback = np.nan
        return num_greenback
