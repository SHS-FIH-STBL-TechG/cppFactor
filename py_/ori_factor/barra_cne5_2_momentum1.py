...
class barra_cne5_2_momentum1(BaseFactor):
    factor_type = "DAY"
    ...
    def calc_single(self, database):
        ret = database.depend_data["FactorData.Basic_factor.pct_chg"].iloc[:-20]
        ...
        # ret = ret.clip(-0.3,0.3)  # 收益去极值,去极值会降低相关性
        ret = ret * ((ret.count() > 500) + 0).replace(0, np.nan) #加上后相关性降低
        mom = np.log((ret / 100 + 1)).ewm(halflife=126).sum().iloc[-1]  # 转化为对数收益率：可加性
        def winsor(beta):
            beta1 = beta.mask(beta.mean - 1 * beta.std(), beta.mean() + 1 * beta.std())  # trimming
