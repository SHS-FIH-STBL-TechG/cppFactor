...
class m_vpc_mut_ty_log73(BaseFactor):
    #  定义因子参数
    ...
    depend_data = ['FactorData.Basic_factor.buytradeamt_minute',
                   ...
                   'FactorData.Basic_factor.selltradenum_minute',
    ...
    def calc_single(self, database):
        ...
        s_num = database.depend_data["FactorData.Basic_factor.selltradenum_minute"].copy().iloc[-237*self.lag:,:]
        ...
        s_num = data_filter(s_num, limit_status, method='minute')
        filter_index = b_amt.resample(period, how='last').dropna(how='all').index
        b_amt_min = b_amt.resample(period).sum().reindex(filter_index)
        b_num_min = b_num.resample(period).sum().reindex(filter_index)
        s_amt_min = s_amt.resample(period).sum().reindex(filter_index)
        s_num_min = s_num.resample(period).sum().reindex(filter_index)
        b_amt_per = b_amt_min / b_num_min
        s_amt_per = s_amt_min / s_num_min
        ratio = b_amt_per / s_amt_per
        ans = ratio.mean()
        return ans
