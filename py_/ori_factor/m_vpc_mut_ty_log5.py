...
class m_vpc_mut_ty_log5(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        close_df_lastday = close_df.iloc[-237:,:]
        ...
        closedfbeyonduplimit = pd.DataFrame(isbeyonduplimit.values*close_df_lastday.values,index = isbeyonduplimit.index,columns=isbeyonduplimit.columns)
        closedfbeyonduplimit = closedfbeyonduplimit.T.div(close_df_lastday.mean(),axis=0).T
        alphastat=closedfbeyonduplimit.mean()

        return alphastat

    def reform(self, temp_result):
        factor_values = temp_result
        factor_values_mean = factor_values.rolling(window=self.reform_window, min_periods=1).max()
        return factor_values_mean
