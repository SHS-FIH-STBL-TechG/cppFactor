...
class m_vpc_mut_ty_log15(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        stdstm = ret_adj.resample('30min', closed='right', label='right').std().dropna(how='all')
        avgstm = ret_adj.resample('30min', closed='right', label='right').mean().dropna(how='all')
        avgstm = avgstm.reindex(stdstm.index)
        adj_std = pd.DataFrame(stdstm.values * np.sign(avgstm).values, index=stdstm.index, columns=stdstm.columns)
        return adj_std.mean(axis=0)
