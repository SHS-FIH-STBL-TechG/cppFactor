...
class m_vpc_mut_ty_log74(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.amt_minute","FactorData.Basic_factor.amt_by_yuan", "FactorData.Basic_factor.limit_status_minute"]
    ...
    def calc_single(self, database):
        ...
        day_amt = database.depend_data['FactorData.Basic_factor.amt_by_yuan']
        amt_ratio = minute_amt.mean(axis=0) / day_amt.mean(axis=0)
        return amt_ratio
