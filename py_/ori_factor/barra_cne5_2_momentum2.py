...
class barra_cne5_2_momentum2(BaseFactor):
    factor_type = "DAY"
    ...
    def calc_single(self, database):
        ...
        rf = pd.DataFrame(np.random.rand(len(ret.iloc[:,0])), index = ret.index) / 252
        ret = np.subtract(ret, rf)

        # ret = ret.clip(-0.25,0.25)  # 收益去极值
        ret = ret * ((close.count() > 500) + 0).replace(0, np.nan) #加上后相关性降低
