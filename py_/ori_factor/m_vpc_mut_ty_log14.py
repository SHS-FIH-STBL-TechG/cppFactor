...
class m_vpc_mut_ty_log14(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        std = ret_adj.groupby(date).std()
        avg = ret_adj.groupby(date).mean()
        adj_std = pd.DataFrame(std.values*np.sign(avg).values,index = std.index,columns = std.columns)
