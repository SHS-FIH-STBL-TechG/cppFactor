...
class m_vpc_mut_ty_log68(BaseFactor):
    #  定义因子参数
    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    ...
    def calc_single(self, database):
        ...
        alpha = amt.rolling(window=5, min_periods=2).mean().kurt()
        return alpha

    def reform(self, temp_result):
        alpha = temp_result.rolling(window=5, min_periods=1).apply(np.nanmean, raw=True)
        return alpha
