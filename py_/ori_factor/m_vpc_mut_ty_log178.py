...
class m_vpc_mut_ty_log178(BaseFactor):
    #  定义因子参数
    ...
    lag = 10
    reform_window=1
    batch_info={'sub_window':20,'daily_split_way':'bw','intraday_stat':'kurt','daily_stat':'mean'}
    ...
    def calc_single(self, database):
        ...
        min_close = self.data_filter2(min_close, limit_status, method='minute')
        min_close = min_close.iloc[-237 * (self.lag):, :]
        fordate = deepcopy(min_close)
        intraday_stat = self.batch_info['intraday_stat']
        daily_stat = self.batch_info['daily_stat']
        sub_window = self.batch_info['sub_window']
        daily_split_way=self.batch_info['daily_split_way']
        ...
        datetime_df.index = datetime_df['datetime']
        # 计算局部信号
        minclose_1 = min_close.groupby(datetime_df['date']).first()
        minclose_1.index = datetime_df['datetime'].groupby(datetime_df['date']).first()
        minclose_1_div = minclose_1.reindex(min_close.index).fillna(method='ffill')
        mincumret_daily = pd.DataFrame(min_close.values/minclose_1_div.values,index = min_close.index,columns =min_close.columns)
        mincumret_daily_cummax = mincumret_daily.groupby(datetime_df['date']).cummax()
        drawdown = pd.DataFrame(mincumret_daily.values-mincumret_daily_cummax.values,index = mincumret_daily.index,columns=mincumret_daily.columns)
        # subinfo_df =datetime_df.groupby(['date','time_id']).apply(lambda x:calc_intra_subinfo(x,min_ret))
        grper = pd.Series(list(zip(datetime_df['date'],datetime_df['time_id'])),index= datetime_df.index)
        subinfo_df = drawdown.groupby(grper).min()
        # 计算单日日内特征
        intrafun_choice = {
            'mean':get_mean,
            'std':get_std,
        ...
        date_sidx = subinfo_df.reset_index()['index'].apply(lambda x:x[0])
        date_sidx.index =  subinfo_df.index#提取局部信号的multiindex信息，为下一步进行日间的聚合
        dailyinfo_df = subinfo_df.groupby(date_sidx).apply(lambda x:intrafun_choice.get(intraday_stat,raise_error)(x))
