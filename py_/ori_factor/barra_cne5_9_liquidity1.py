...
class barra_cne5_9_liquidity1(BaseFactor):
    factor_type = "DAY"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.free_turn", "FactorData.Basic_factor.mkt_cap_ard",
    ...
    def calc_single(self, database):
        ...
        def winsor(beta):
            ...
            return beta1
        turn = turn * ((turn.count() > 60) + 0).replace(0, np.nan)
        ...
        weight = np.log(float.iloc[-1]).loc[x.index]
