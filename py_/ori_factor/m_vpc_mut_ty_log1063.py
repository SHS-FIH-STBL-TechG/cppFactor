...
class m_vpc_mut_ty_log1063(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "FIX"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.close_minute","FactorData.Basic_factor.limit_status_minute","FactorData.Basic_factor.volume_minute"]
    # 计算每个时点的因子所需要前移的数据窗口大小
    # 例如，为日频因子，lag=3表示计算某一日的因子值需要依赖前三个交易日和当日的数据，默认为0，可不设置
    lag = 10
    reform_window=1
    batch_info={'sub_window':20,'daily_split_way':'bw','intraday_stat':'kurt','daily_stat':'skew'}

    # 每次播放的计算具体方法。必须实现。
    def calc_single(self, database):
        #分钟线242转换为240或者241根，operation为list，默认为["merge", "merge"],第一位表示对925时刻处理，第二位表示对1500处理
        #处理方式可分为"merge"、"drop"和"",分别表示合并、删除、和不操作。优化后单次播放时分钟
        ...
        def calc_intra_subinfo(group,min_ret,min_volume):
            subidx = group['datetime'].values
            min_ret_sub = min_ret.loc[subidx]
            min_vol_sub = min_volume.loc[subidx]
            mvd_sub = min_vol_sub.values*(min_ret_sub.values>0)
            subinfo = pd.Series(np.nanstd(mvd_sub,axis=0),index = min_ret_sub.columns)
            return subinfo
        ...
        database.depend_data['FactorData.Basic_factor.ret_minute'] = self.data_filter2(database.depend_data['FactorData.Basic_factor.ret_minute'], limit_status, method='minute')

        database.depend_data['FactorData.Basic_factor.close_minute'] = self.data_filter2(database.depend_data['FactorData.Basic_factor.close_minute'], limit_status, method='minute')

        database.depend_data['FactorData.Basic_factor.volume_minute'] = self.data_filter2(database.depend_data['FactorData.Basic_factor.volume_minute'], limit_status, method='minute')
        minute_data_transform(database.depend_data, operation=["drop1", "drop4"])
        min_volume = database.depend_data['FactorData.Basic_factor.volume_minute']
        min_ret = database.depend_data['FactorData.Basic_factor.ret_minute']
        min_ret = min_ret.iloc[237:,:]
        min_close= min_forward_adj(database.depend_data['FactorData.Basic_factor.close_minute'])
        ...
        # 计算局部信号
        subinfo_df =datetime_df.groupby(['date','time_id']).apply(lambda x:calc_intra_subinfo(x,min_ret,min_volume))
