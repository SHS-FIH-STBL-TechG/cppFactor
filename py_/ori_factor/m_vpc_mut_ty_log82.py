...
class m_vpc_mut_ty_log82(BaseFactor):
    #  定义因子参数
    ...
    def calc_single(self, database):
        ...
        tradenum = database.depend_data["FactorData.Basic_factor.tradenum_minute"].iloc[-237*self.lag:, :].copy()
        close = min_forward_adj(database.depend_data["FactorData.Basic_factor.close_minute"].iloc[-237*self.lag:, :].copy())
        ...
        tradenum = data_filter(tradenum,limit_status,method='minute')
        close = data_filter(close,limit_status,method='minute')
        apt = amt.sum(axis=0, skipna=True)/tradenum.sum(axis=0, skipna=True)
        ret = np.log(close/close.shift(1))
        index = np.zeros(ret.shape)
        index[ret.values < 0] = 1
        apt_flow = (amt*index).sum(axis=0, skipna=True)/(tradenum*index).sum(axis=0, skipna=True)
        ans = apt_flow/apt
        return ans
