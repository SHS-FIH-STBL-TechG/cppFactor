...
class m_vpc_mut_ty_log47(BaseFactor):
    #  定义因子参数
    ...
    lag = 10
    reform_window=1
    batch_info={'sub_window':20,'daily_split_way':'nt','intraday_stat':'mean','daily_stat':'kurt'}

    # 每次播放的计算具体方法。必须实现。
    def calc_single(self, database):
        logger.info("inside m_vpc_mut_ty_log47.py calc_single started...")
        #分钟线242转换为240或者241根，operation为list，默认为["merge", "merge"],第一位表示对925时刻处理，第二位表示对1500处理
        #处理方式可分为"merge"、"drop"和"",分别表示合并、删除、和不操作。优化后单次播放时分钟线转换速度为毫秒级
        minute_data_transform(database.depend_data, operation = ["drop1", "drop4"])
        logger.info("inside m_vpc_mut_ty_log47.py minute_data_transform done.")
        ...
        def calc_intra_subinfo(group,min_amt):
            ...
            min_amt_sub = min_amt.loc[subidx]
            subinfo = pd.Series(np.nanmean(min_amt_sub,axis=0),index = min_amt_sub.columns)
            return subinfo

        # 提取数据
        min_amt = database.depend_data['FactorData.Basic_factor.amt_minute'];limit_status = database.depend_data['FactorData.Basic_factor.limit_status_minute']
        min_amt = self.data_filter2(min_amt, limit_status, method='minute')
        fordate = deepcopy(min_amt)
        ...
        logger.info("inside m_vpc_mut_ty_log47.py 提取数据 done.")
        ...
        logger.info("inside m_vpc_mut_ty_log47.py 获取日期和局部信号对应的index done.")

        # 计算局部信号
        subinfo_df =datetime_df.groupby(['date','time_id']).apply(lambda x:calc_intra_subinfo(x,min_amt))
        logger.info("inside m_vpc_mut_ty_log47.py 计算局部信号 done.")
        ...
        logger.info("inside m_vpc_mut_ty_log47.py 计算单日日内特征 done.")
        ...
        logger.info("inside m_vpc_mut_ty_log47.py 计算日间信号 done.")
        return alpha

    def reform(self, temp_result):
        logger.info("inside m_vpc_mut_ty_log47.py reform started...")
        alpha = temp_result
        logger.info("inside m_vpc_mut_ty_log47.py reform done.")
        return alpha
